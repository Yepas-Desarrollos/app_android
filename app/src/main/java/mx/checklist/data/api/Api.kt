...
interface Api {
    ...
    // Listar revisiones pendientes de corrección (SUPERVISOR)
    @GET("audit-reviews/pending")
    ...
    // Listar revisiones corregidas para validar (AUDITOR)
    @GET("audit-reviews/corrected")
    ...
    suspend fun getReviewDetail(@Path("id") reviewId: Long): ReviewDetailDto

    // Validar una corrección (aprobar o rechazar)
    @PATCH("audit-reviews/{id}/validate")
    suspend fun validateCorrection(
        @Path("id") reviewId: Long,
        @Body request: ValidateCorrectionRequest
    ...
    // Listar MIS correcciones (SUPERVISOR) - historial de lo que he corregido
    @GET("audit-corrections/mine")
    suspend fun getMyCorrections(
        @Query("limit") limit: Int = 20,
        @Query("page") page: Int = 1,
        @Query("status") status: String? = null // CORRECTED, VALIDATED, REJECTED, or all
